...
const startTheia = async ({ containerId }, { headers }) => {
  const port = 1051;
  const runCmd = ['/bin/sh', '-c', `bouncecode theia start --port ${port}`];
  const waitCmd = ['/bin/sh', '-c', `bouncecode wait --port ${port}`];
  const endpointCmd = ['/bin/sh', '-c', `echo http://${port}.$DOMAIN`];
...
const stopTheia = async ({ containerId }, { headers }) => {
  const port = 1051;
  const cmd = ['/bin/sh', '-c', `bouncecode theia stop --port ${port}`];
...
const startJupyter = async ({ containerId }, { headers }) => {
  const port = 1052;
  const runCmd = ['/bin/sh', '-c', `bouncecode jupyter start --port ${port}`];
  const waitCmd = ['/bin/sh', '-c', `bouncecode wait --port ${port}`];
  const endpointCmd = ['/bin/sh', '-c', `echo http://${port}.$DOMAIN`];
  const {execInspect} = await runExec({ containerId, cmd: runCmd }, { headers });
  await waitExec({ containerId, cmd: waitCmd }, { headers });
...
const stopJupyter = async ({ containerId }, { headers }) => {
  const port = 1052;
  const cmd = ['/bin/sh', '-c', `bouncecode jupyter stop --port ${port}`];
  const {execOutput, execInspect} = await waitExec({ containerId, cmd }, { headers });
  return {output: execOutput, exec: execInspect};
}
...
const startVnc = async ({ containerId }, { headers }) => {
  const port = 1071;
  const vncPort = 5901;
  const runCmd = ['/bin/sh', '-c', `bouncecode vnc start --port ${port} --vnc-port ${vncPort}`];
  const waitCmd = ['/bin/sh', '-c', `bouncecode wait --port ${port}`];
  const endpointCmd = ['/bin/sh', '-c', `echo http://${port}.$DOMAIN/vnc_blank.html`];
  const {execInspect} = await runExec({ containerId, cmd: runCmd }, { headers });
  await waitExec({ containerId, cmd: waitCmd }, { headers });
  const {execOutput} = await waitExec({ containerId, cmd: endpointCmd }, { headers });
  return {output: execOutput ? execOutput.trim() : execOutput, exec: execInspect};
...
const stopVnc = async ({ containerId }, { headers }) => {
  const port = 1071;
  const vncPort = 5901;
  const cmd = ['/bin/sh', '-c', `bouncecode vnc stop --port ${port} --vnc-port ${vncPort}`];
  const {execOutput, execInspect} = await waitExec({ containerId, cmd }, { headers });
  return {output: execOutput, exec: execInspect};
}
